...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (!deepseekResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: `DeepSeek API error: ${deepseekResponse.status}`, detail: errText }), {
        status: deepseekResponse.status,
        headers: corsHeaders
      });
    }
    const result = await deepseekResponse.json();
    const content = result.choices?.[0]?.message?.content || "\u62B1\u6B49\uFF0C\u6682\u65F6\u65E0\u6CD5\u83B7\u53D6\u56DE\u590D";
    return new Response(JSON.stringify({ success: true, content, model: result.model }), { headers: corsHeaders });
  } catch (error) {
...
// ../.wrangler/tmp/pages-PuIDqY/functionsRoutes-0.6623226136999627.mjs
var routes = [
  {
    routePath: "/api/chat",
    mountPath: "/api",
