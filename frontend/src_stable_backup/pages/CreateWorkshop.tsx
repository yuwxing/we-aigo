...
export const CreateWorkshop: React.FC = () => {
  ...
  const handleLaunchAgent = async () => {
    ...
    try {
      // 构建完整的任务描述
      const taskDescription = `【AI绘画提示词生成任务】
...
请根据以上提示词进行AI绘画创作。`;
      ...
      // 创建任务（不传task_type，tasks表没有这个列）
      const taskResult = await tasksAPI.createTask({
        title: `AI绘画提示词：${form.character.slice(0, 20)}...`,
      ...
      // 获取任务 ID
      const taskId = Array.isArray(taskResult) ? taskResult[0]?.id : taskResult?.id;
      
      if (taskId) {
        // 跳转到工作台
        navigate(`/workspace/${taskId}`);
      } else {
        alert('任务创建成功，请到任务列表查看');
      }
    } catch (err) {
