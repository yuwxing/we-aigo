      ...
      await fetch(`${SUPABASE_URL}tasks`, {
        ...
        body: JSON.stringify({
          ...
          description: JSON.stringify({
            ...
            submitterName: currentUser?.username || '匿名用户',
            submittedAt: now,
            source: '平台反馈按钮'
          }),
            ...
            {submitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-600 mt-2">
                  {claimEnabled && claimAmount > 0 ? (
                    <>
                      已提交！审核通过后<br/>
                      <span className="text-emerald-600 font-bold text-lg">{claimAmount.toLocaleString()} Token</span><br/>
                      将自动到账，可在余额页面查看
                    </>
                  ) : (
                    '反馈收集师·倾听者已收到，将尽快处理'
              ...
              <>
                <div className="text-center mb-4">
                  <h3 className="text-2xl font-bold text-slate-900">🔍 发现错误，赚Token！</h3>
                  <p className="text-sm text-slate-500 mt-1">平台承诺：虚假信息赔1000token，Bug按程度赔50-1000token</p>
                </div>
                ...
                {selectedType?.claimable && (
                  <div className="mb-5 p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl">
                    ...
                    {claimEnabled && (
                      <>
                        <div className="mb-3">
                          <label className="block text-xs text-slate-600 mb-1.5">赔付金额（Token）</label>
