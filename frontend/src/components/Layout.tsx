...
// 维护公告：改这里就能显示/隐藏公告条
const SITE_NOTICE = ''; // 留空=不显示，填内容=显示滚动公告，如：'🔧 平台升级中，部分功能可能暂时不可用，预计明早恢复'
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('balance-refresh', handleRefreshBalance);
    return () => {
      window.removeEventListener('balance-refresh', handleRefreshBalance);
    };
  }, [user]);
  ...
  return (
    <div className="min-h-screen page-wrapper">
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {user ? (
                <>
                  ...
                  >
                    <img src="/weg-coin.png" alt="WEG币" className="w-6 h-6 rounded-full" style={{ objectFit: 'cover' }} />
              ...
              {user ? (
                <div className="flex items-center justify-between px-4 py-3 rounded-xl mb-4 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100">
                  <Link to="/balance" className="flex items-center gap-3">
                    <img src="/weg-coin.png" alt="WEG币" className="w-10 h-10 rounded-xl" style={{ objectFit: 'cover' }} />
                    <div>
                      <p className="text-xs text-slate-500">WEG币 余额</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="ml-2 flex items-center gap-1 px-2 py-1 bg-red-50 text-red-500 rounded-lg text-xs font-medium hover:bg-red-100 transition-colors border border-red-200"
                      title="退出登录"
                    >
      ...
      {/* 底部导航栏 - 梦幻风格 */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 glass-card border-t border-purple-100/50 z-50 safe-area-inset-bottom">
        <div style={{ display: 'flex', padding: '6px 4px', gap: '0px', width: '100%', boxSizing: 'border-box' }}>
          {bottomNavItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              style={{ flex: '1 1 0%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2px', padding: '6px 2px', borderRadius: '12px', textDecoration: 'none', transition: 'all 0.2s', minWidth: 0 }}
              className={clsx(
                item.highlight
            ...
            >
              {item.highlight ? (
                ...
                <>
                  {item.icon}
                  <span style={{ fontSize: '10px', fontWeight: '500', maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: '1.2' }}>{item.label}</span>
                </>
              )}
            </Link>
          ))}
