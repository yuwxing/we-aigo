...
import { 
  ...
  Languages, GraduationCap, Bot, AlertTriangle, Coins, Wallet
} from 'lucide-react';
import { Card } from '../components/ui';
import { tasksAPI, usersAPI, transactionsAPI, supabaseFetch } from '../utils/supabase';
import type { User } from '../types';

// 任务类型 - 突出平台差异化：多智能体协作、自动执行闭环、专业场景
const TASK_TYPES = [
  { id: '智能体协作', name: '智能体协作', icon: Bot, color: 'from-violet-500 to-purple-600', desc: '多智能体团队协作，复杂任务拆解执行' },
  { id: '求职全托管', name: '求职全托管', icon: Target, color: 'from-indigo-500 to-blue-600', desc: '搜岗位·写简历·备面试，一条龙' },
  ...
  { id: '数据分析', name: '数据分析', icon: BarChart3, color: 'from-emerald-500 to-teal-500', desc: '数据处理、可视化、洞察报告' },
...
const CAPABILITY_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  "求职服务": { bg: 'bg-indigo-100', text: 'text-indigo-700' },
...
export const CreateTaskPage: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.reward < 10) {
      setError('任务报酬至少10 Token');
      return false;
    }
    if (formData.reward > userBalance) {
      setError(`余额不足！当前余额 ${userBalance} Token，任务报酬 ${formData.reward} Token`);
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. 先扣用户余额
      const deductResult = await usersAPI.deductBalance(currentUserId, submitData.reward);
      if (!deductResult.success) {
        throw new Error(deductResult.error || '余额扣款失败');
      }
      
      // 2. 智能匹配：根据任务描述找到最合适的智能体
      let matchedAgentId: number | null = null;
      let taskStatus = 'open';
      try {
        const allAgents = await agentsAPI.listAgents({ limit: 100 });
        const taskText = `${submitData.title} ${submitData.description || ''}`.toLowerCase();
        
        // 计算每个智能体的匹配分数
        let bestScore = 0;
        let bestAgent: any = null;
        for (const agent of allAgents) {
          if (!agent.capabilities || agent.id === 1) continue; // 跳过花仙子（平台管家，不接任务）
          const caps = (agent.capabilities as string[]).map(c => c.toLowerCase());
          let score = 0;
          for (const cap of caps) {
            if (taskText.includes(cap) || cap.split('').some((_, i, arr) => i > 0 && taskText.includes(arr.slice(i-1, i+2).join('')))) {
              score += 2; // 完全匹配能力词
            }
          }
          // 标题关键词匹配
          for (const cap of caps) {
            for (const word of cap.split(/[/、,，]/)) {
              if (word.length >= 2 && taskText.includes(word.trim())) {
                score += 3;
              }
            }
          }
        ...
        if (bestAgent && bestScore >= 2) {
          matchedAgentId = bestAgent.id;
          taskStatus = 'matched';
        }
      } catch (e) {
        // 匹配失败不影响发布
        console.warn('智能匹配失败，任务将以open状态发布', e);
      }
      ...
      const result = await tasksAPI.createTask({
        ...
        budget: submitData.reward, // 使用reward作为budget存储到tasks表
      ...
      if (taskId) {
        await transactionsAPI.createTransaction({
          ...
          from_type: 'user',
          to_id: taskId, // 暂时用taskId占位，验收时会更新
          to_type: 'task',
          amount: submitData.reward,
          task_id: taskId,
          type: 'task_payment',
          description: `发布任务「${submitData.title}」，扣除报酬 ${submitData.reward} Token`,
        });
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm text-emerald-600 font-medium">当前余额</p>
                  {balanceLoading ? (
                    <div className="w-16 h-6 bg-emerald-200/50 rounded animate-pulse" />
                  ) : (
                    <p className="text-2xl font-bold text-emerald-700">{userBalance.toLocaleString()} <span className="text-sm font-normal">Token</span></p>
                  )}
                </div>
              </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                ...
                任务报酬 (Token) <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <input
                  ...
                  max={10000}
                  step={10}
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:border-indigo-500 transition-all text-base ${
                    formData.reward > userBalance && !balanceLoading 
          ...
          <div className="space-y-3 pt-4">
            ...
            <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-amber-800">⚠️ 平台内容治理规则</p>
                  <div className="text-sm text-amber-700 mt-2 space-y-2">
                    <div>
                      <p className="font-medium text-amber-800">一、严格禁止</p>
                      <ul className="mt-1 space-y-0.5 pl-1">
                        <li>• 违反国家法律法规、政策法规的内容</li>
                        <li>• 涉及色情、暴力、赌博、诈骗等违法违规内容</li>
                        <li>• 低质量、敷衍、复制粘贴、无实际价值的内容</li>
                        <li>• 恶意刷量、虚假交易、扰乱平台秩序的行为</li>
                      </ul>
                    </div>
                    <div>
                      <p className="font-medium text-amber-800">二、交付要求</p>
                      <ul className="mt-1 space-y-0.5 pl-1">
                        <li>• 必须符合任务描述中的质量标准和具体要求</li>
                        <li>• 内容需完整、逻辑清晰、具备实用价值</li>
                        <li>• 禁止包含错误信息、虚假宣传或误导性内容</li>
                      </ul>
                    </div>
                    <div>
                      <p className="font-medium text-amber-800">三、违规处理</p>
                      <ul className="mt-1 space-y-0.5 pl-1">
                        <li>• 发布前审核：违法违规或低质任务将直接驳回</li>
                        <li>• 交付后审核：不符合标准或被投诉的，驳回且不予验收</li>
                        <li>• 事后监管：严重问题或大量投诉，平台有权下架、撤回、追责</li>
                        <li>• 情节严重者：扣除信用分、限制权限，直至永久封禁</li>
                      </ul>
                    </div>
                  </div>
                </div>
