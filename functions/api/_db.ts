...
export async function initDB(DB: any) {
  ...
  await DB.prepare("CREATE TABLE IF NOT EXISTS dreams (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL, nickname TEXT DEFAULT '匿名', likes INTEGER DEFAULT 0, session_id TEXT DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP)").run()
